...
def createArticulationBellow(OuterRadius, NBellows, BellowHeight, TeethRadius, FingerWidth, PlateauHeight, lc=1):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # MODIFICACIÓN: Aplicar la Ranura Superior al Sólido del Fuelle (Paso 4)
    # ─────────────────────────────────────────────────────────────────────────
    AnchoRanura = 4.0        
    ProfundidadRanura = 3.5  
    Y_inicio_corte = OuterRadius - ProfundidadRanura
    Alt_corte = ProfundidadRanura + 10.0

    CajaRanuraTag = gmsh.model.occ.addBox(
        -AnchoRanura / 2, Y_inicio_corte, -TotalBellowHeight,
        AnchoRanura, Alt_corte, TotalBellowHeight * 2
    )
...
def createCavityVolume(OuterRadius, NBellows, BellowHeight, TeethRadius, WallThickness, CenterThickness, CavityCorkThickness, PlateauHeight):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # MODIFICACIÓN: Recorte de la Cavidad Interna (Paso 6)
    # ─────────────────────────────────────────────────────────────────────────
    AnchoRanura = 4.0        
    ProfundidadRanura = 3.5  
    
    # Aplicamos tus deltas absolutos en mm para proteger el espesor de la silicona
    AnchoRanura += 2.0    
    Y_inicio_corte = (OuterRadius - ProfundidadRanura) - 1.0 
    Alt_corte = ProfundidadRanura + 10.0

    CajaCorteIntTag = gmsh.model.occ.addBox(
        (-AnchoRanura / 2), Y_inicio_corte, -(TotalHeight + 1),
        AnchoRanura, Alt_corte, 2 * (TotalHeight + 1)
    )
...
def buildCableChannelParams(SegmentIndex, Length, NBellows, BellowHeight,
                            CavityCorkThickness,
                            CableWidth=2.0, CableHeight=None,
                            XOffset=0.0):
    if CableHeight is None:
        CableHeight = CavityCorkThickness + 1.0
...
def createFinger(Length, Height, OuterRadius, TeethRadius, PlateauHeight, JointHeight,
    ...
    Bellow1DimTags = createArticulationBellow(OuterRadius, NBellows, BellowHeight,
                                              TeethRadius, Thickness, PlateauHeight, lc=lc)
    ...
    AllCavitiesDimTags = createAllCavities(
        ...
        WallThickness, CenterThickness, CavityCorkThickness, PlateauHeight, lc=lc
    )
...
def Finger(Length, Height, OuterRadius, TeethRadius, PlateauHeight, JointHeight,
           ...
           Stage1Mod=False, RefineAroundCavities=False,
           CableChannelParams=None):
    ...
    FingerDimTag = createFinger(
        ...
        CenterThickness, CavityCorkThickness, lc,
        Stage1Mod=Stage1Mod,
        CableChannelParams=CableChannelParams,   
    )
