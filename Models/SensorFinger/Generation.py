...
def createArticulationBellow(OuterRadius, NBellows, BellowHeight, TeethRadius, FingerWidth, PlateauHeight, 
                             slot_width=4.0, slot_depth=3.5, lc=1):
    PointTags = np.empty((0,1), int)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PARAMETRIC MODIFICATION: Cut out upper slot from bellow stack surface
    # ─────────────────────────────────────────────────────────────────────────
    Y_inicio_corte = OuterRadius - slot_depth
    Alt_corte = slot_depth + 10.0

    CajaRanuraTag = gmsh.model.occ.addBox(
        -slot_width / 2, Y_inicio_corte, -TotalBellowHeight,
        slot_width, Alt_corte, TotalBellowHeight * 2
    )
    CajaRanuraDimTag = (3, CajaRanuraTag)
    
    CutBellowOut = gmsh.model.occ.cut(BellowDimTags, [CajaRanuraDimTag], removeObject=True, removeTool=True)
    BellowDimTags = CutBellowOut[0]
    # ─────────────────────────────────────────────────────────────────────────
...
def createCavityVolume(OuterRadius, NBellows, BellowHeight, TeethRadius, WallThickness, CenterThickness, 
                       CavityCorkThickness, PlateauHeight, slot_width=4.0, slot_depth=3.5):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PARAMETRIC MODIFICATION: Cut out and flatten internal air cavity volume
    # ─────────────────────────────────────────────────────────────────────────
    AnchoCajaInterna = slot_width + 2.0    
    Y_inicio_corte_int = (OuterRadius - slot_depth) - 1.0 
    Alt_corte_int = slot_depth + 10.0

    CajaCorteIntTag = gmsh.model.occ.addBox(
        (-AnchoCajaInterna / 2), Y_inicio_corte_int, -(TotalHeight + 1),
        AnchoCajaInterna, Alt_corte_int, 2 * (TotalHeight + 1)
    )
    CajaCorteIntDimTag = (3, CajaCorteIntTag)
    
    CutCavityOut = gmsh.model.occ.cut(CavityBaseDimTag, [CajaCorteIntDimTag], removeObject=True, removeTool=True)
    CavityBaseDimTag = CutCavityOut[0]
    # ─────────────────────────────────────────────────────────────────────────
...
def createAllCavities(Length, OuterRadius, NBellows, BellowHeight, TeethRadius, WallThickness, CenterThickness, 
                      CavityCorkThickness, PlateauHeight, slot_width=4.0, slot_depth=3.5, lc=1):
    BellowGap = (NBellows-1)*BellowHeight
    Cavity1DimTags = createCavityVolume(OuterRadius, NBellows, BellowHeight, TeethRadius, WallThickness, 
                                        CenterThickness, CavityCorkThickness, PlateauHeight, 
...
def createCavity(OuterRadius, NBellows, BellowHeight, TeethRadius, WallThickness, CenterThickness, 
                 CavityCorkThickness, PlateauHeight, Z_translation, slot_width=4.0, slot_depth=3.5):
...
# ─────────────────────────────────────────────────────────────────────────────
# Cable Channel Utilities (Updated for parametric integration)
# ─────────────────────────────────────────────────────────────────────────────

def getCavitySegmentZCenter(SegmentIndex, Length, NBellows, BellowHeight):
    BellowGap = (NBellows - 1) * BellowHeight
    if SegmentIndex == 1:
        return -(Length + BellowGap / 2)
    elif SegmentIndex == 2:
        return -(2 * Length + 3 * BellowGap / 2)
    else:
        raise ValueError("SegmentIndex must be 1 or 2")


def buildCableChannelParams(SegmentIndex, Length, NBellows, BellowHeight,
                            CavityCorkThickness, OuterRadius, slot_depth,
                            CableWidth=2.0, CableHeight=5.0, XOffset=0.0):
    """
    Convenience helper dynamically adjusted based on slot_depth.
    Locks the cable execution directly inside the flattened slot bottom.
    """
    TotalBellowExtent = NBellows * BellowHeight
    ZCenter = getCavitySegmentZCenter(SegmentIndex, Length, NBellows, BellowHeight)

    # Starting coordinates inside the flattened surface base
    Y_inicio_cable = OuterRadius - slot_depth

    return {
        'X':  XOffset - CableWidth / 2,
        'Y':  Y_inicio_cable,                    
        'Z':  ZCenter + TotalBellowExtent / 2,   
        'dX': CableWidth,
        'dY': -CableHeight,                      # Inward extrusion projection (-Y)
        'dZ': -TotalBellowExtent,                
    }


# ─────────────────────────────────────────────────────────────────────────────
# Core Finger Creation
# ─────────────────────────────────────────────────────────────────────────────
...
def createFinger(Length, Height, OuterRadius, TeethRadius, PlateauHeight, JointHeight,
                 ...
                 CenterThickness, CavityCorkThickness, lc, slot_width=4.0, slot_depth=3.5, 
                 Stage1Mod=False, CableChannelParams=None):
    ...
    if Stage1Mod:
        Segment2DimTags = [makeSegmentStage1Mod(Segment2DimTags[0], Length, Height,
                                                 JointHeight, Thickness, JointSlopeAngle, lc=1)]

    Segment3DimTags = gmsh.model.occ.copy(Segment2DimTags)
    Segment1DimTag = makeSegmentFixationMod(Segment1DimTag, Length, Height, JointHeight,
                                             Thickness, JointSlopeAngle, FixationWidth)
    ...
    #-------------------
    # Bellows (Propagates slot dimensions)
    #-------------------
    Bellow1DimTags = createArticulationBellow(OuterRadius, NBellows, BellowHeight,
                                              TeethRadius, Thickness, PlateauHeight, 
                                              slot_width=slot_width, slot_depth=slot_depth, lc=lc)
    ...
    FuseOut = gmsh.model.occ.fuse(
        Bellow1DimTags,
        [Segment1DimTag] + Segment2DimTags + Segment3DimTags + Bellow2DimTags
    )
    FingerNoCavitiesDimTag = FuseOut[0]
    gmsh.model.occ.synchronize()

    #-------------------
    # Cavities (Propagates slot dimensions)
    #-------------------
    AllCavitiesDimTags = createAllCavities(
        Length, OuterRadius, NBellows, BellowHeight, TeethRadius,
        WallThickness, CenterThickness, CavityCorkThickness, PlateauHeight, 
        slot_width=slot_width, slot_depth=slot_depth, lc=lc
    )

    #-------------------
    # Cut Cavities  (+ optional cable channel)
    #-------------------
    if CableChannelParams is not None:
        CableBoxForCavity = (3, gmsh.model.occ.addBox(
            CableChannelParams['X'],
            CableChannelParams['Y'],
            CableChannelParams['Z'],
            CableChannelParams['dX'],
            CableChannelParams['dY'],
            CableChannelParams['dZ'],
        ))

        CableBoxForFinger = gmsh.model.occ.copy([CableBoxForCavity])

        print("[CableChannel] Sealing cavity at cable location...")
        CutCavitiesOut = gmsh.model.occ.cut(AllCavitiesDimTags, [CableBoxForCavity])
        ModifiedCavitiesDimTags = CutCavitiesOut[0]
        if not ModifiedCavitiesDimTags:
            print("[CableChannel] Warning: cable box did not intersect any cavity. Proceeding without cavity modification.")
            ModifiedCavitiesDimTags = AllCavitiesDimTags

        CutOut = gmsh.model.occ.cut(FingerNoCavitiesDimTag, ModifiedCavitiesDimTags)
        FingerDimTag = CutOut[0][0]
        gmsh.model.occ.synchronize()

        print("[CableChannel] Cutting cable channel from finger body...")
        CutOut2 = gmsh.model.occ.cut([FingerDimTag], CableBoxForFinger)
        FingerDimTag = CutOut2[0][0]
        gmsh.model.occ.synchronize()
...
def Cavity(Length, Height, Thickness, OuterRadius, NBellows, BellowHeight, TeethRadius,
           WallThickness, CenterThickness, CavityCorkThickness, PlateauHeight, Z_translation,
           slot_width=4.0, slot_depth=3.5, RefineAroundCavities=False):
    
    createCavity(OuterRadius, NBellows, BellowHeight, TeethRadius, WallThickness,
                 CenterThickness, CavityCorkThickness, PlateauHeight, Z_translation,
                 slot_width=slot_width, slot_depth=slot_depth)
...
def Finger(Length, Height, OuterRadius, TeethRadius, PlateauHeight, JointHeight,
           Thickness, JointSlopeAngle, FixationWidth, BellowHeight, NBellows, WallThickness,
           CenterThickness, CavityCorkThickness, lc, slot_width=4.0, slot_depth=3.5,
           Stage1Mod=False, RefineAroundCavities=False, CableChannelParams=None):
    
    FingerDimTag = createFinger(
        Length, Height, OuterRadius, TeethRadius, PlateauHeight, JointHeight,
        Thickness, JointSlopeAngle, FixationWidth, BellowHeight, NBellows, WallThickness,
        CenterThickness, CavityCorkThickness, lc, 
        slot_width=slot_width, slot_depth=slot_depth,
        Stage1Mod=Stage1Mod, CableChannelParams=CableChannelParams,   
    )
